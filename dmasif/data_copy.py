...
def load_protein_npy(pdb_id, data_dir, center=False, single_pdb=False, flexibility=False, binary = False, atomic = False):
    ...
    if flexibility:
        if atomic:
            atom_flex = 100*tensor(np.load(data_dir / (pdb_id + "_atomflex_atom.npy")))
            #print('Atomic')
        else:
...
class PairData(Data): #TODO: deal flexibility
    ...
    def __cat_dim__(self, key, value, store):
        if ("index" in key) or ("face" in key):
            return 1
        else:
            return 0

def load_protein_pair(pdb_id, data_dir,single_pdb=False, flexibility=False, binary = False, atomic = False):
    """Loads a protein surface mesh and its features"""
    pspl = pdb_id.split("_")
    ...
    try:
        p1 = load_protein_npy(p1_id, data_dir, center=False, single_pdb=single_pdb, flexibility = flexibility, binary = binary, atomic = atomic)
    ...
    try:
        p2 = load_protein_npy(p2_id, data_dir, center=False, single_pdb=single_pdb, flexibility = flexibility, binary = binary, atomic = atomic)
    except FileNotFoundError:
    ...
    if flexibility:
        try:
            atom_flex_p1=p1["atom_flexibility"]
            atom_flex_p2=p2["atom_flexibility"]
        except FileNotFoundError:
            print('notfound')
    else:
        atom_flex_p1=None
...
class ProteinPairsSurfaces(InMemoryDataset):
    ...
    def __init__(self, root, ppi=False, train=True, transform=None, pre_transform=None, flexibility=False, antibody = False, crossval = False, subset = 1,  mix = False, binary = False, atomic = False):
        self.ppi = ppi
    ...
    def process(self):
        ...
        if self.antibody:
            if self.crossval:
                ...
                lists_dir = Path("/disk1/fingerprint/SAbDab_preparation/list_corssval")
            else:
                surf_dir = Path("/disk1/fingerprint/data_preparation/01-benchmark_surfaces")
                protein_dir = Path("/disk1/fingerprint/SAbDab_preparation/all_structures/npys_tot")
                if self.mix:
        ...
        try:
            if self.crossval:
                if self.antibody:
                    training_data = Path(f"/disk1/fingerprint/SAbDab_preparation/list_corssval/training_{self.subset}.txt")
                    test_data = Path(f"/disk1/fingerprint/SAbDab_preparation/list_corssval/test_{self.subset}.txt") 
                else:
                    training_data = Path(f"/disk1/fingerprint/dmasif/lists/training_{self.subset}.txt")
                    test_data = Path(f"/disk1/fingerprint/dmasif/lists/test_{self.subset}.txt") 
            else:
                training_data = lists_dir / "training.txt"
                test_data = lists_dir / "testing.txt"
                
            #training_data = Path(f"/disk1/fingerprint/SAbDab_preparation/list_corssval/training_{self.subset}.txt")
            #test_data = Path(f"/disk1/fingerprint/SAbDab_preparation/list_corssval/test_{self.subset}.txt")

            with open(training_data) as f_tr, open(
                test_data
            ) as f_ts:
        ...
        if self.crossval:
            if self.antibody:
                training_data = Path(f"/disk1/fingerprint/SAbDab_preparation/list_corssval/training_ppi_{self.subset}.txt")
                test_data = Path(f"/disk1/fingerprint/SAbDab_preparation/list_corssval/test_ppi_{self.subset}.txt")
                print(f'Subset: {self.subset}')
            else:
                training_data = Path(f"/disk1/fingerprint/dmasif/lists/training_ppi_{self.subset}.txt")
                test_data = Path(f"/disk1/fingerprint/dmasif/lists/test_ppi_{self.subset}.txt")
                print(f'Subset: {self.subset}') 
        else:
            training_data = lists_dir / "training_ppi.txt"
            test_data = lists_dir / "testing_ppi.txt"
        ...
        for p in training_pairs_list:
            try:
                protein_pair = load_protein_pair(p, protein_dir, flexibility=self.flexibility, binary = self.binary, atomic = self.atomic)
            except FileNotFoundError:
                continue
            except ValueError:
                continue
