...
from data_copy import ProteinPairsSurfaces, CenterPairAtoms
from data_copy import RandomRotationPairAtoms, NormalizeChemFeatures, iface_valid_filter
...
model_path = "/disk1/fingerprint/cv_model/" + args.experiment_name

print(f"Experiment name: {args.experiment_name} | Antibody: {args.antibody} | Flexibility: {args.flexibility} |  Binary: {args.binary} | Atomic: {args.atomic}" )
...
#    args.experiment_name = args.experiment_name + f"_{subset}"
...
if args.antibody:
    if args.mix:
        ...
        folder_data = "/disk1/fingerprint/surface_data_ab_atom_1"
    else:
        folder_data = "/disk1/fingerprint/surface_data_1"
else:
    if args.atomic:
        folder_data = "/disk1/fingerprint/surface_data_atom_1"
    else:
        folder_data = "/disk1/fingerprint/dmasif/surface_data_1"

"""if args.cross_validation:
    if args.antibody:
        folder_data = f"/disk1/fingerprint/surface_data_{args.kfold}"
    else:
        folder_data = f"/disk1/fingerprint/dmasif/surface_data_{args.kfold}"
    print(f'Subset: {args.kfold}')"""

train_dataset = ProteinPairsSurfaces(
            folder_data, ppi=args.search, train=True, transform=transformations, flexibility = args.flexibility, antibody = args.antibody, crossval = args.cross_validation, subset = args.kfold, mix = args.mix, binary = args.binary, atomic = args.atomic
        )
...
test_dataset = ProteinPairsSurfaces(
            folder_data, ppi=args.search, train=False, transform=transformations, flexibility = args.flexibility, mix = args.mix, binary = args.binary, atomic = args.atomic
        )
