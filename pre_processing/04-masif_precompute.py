...
from source.masif_modules.read_data_from_surface import read_data_from_surface, compute_shape_complementarity
...
# Creates a list containing the third command-line argument. This list represents PPI pair identifiers.
...
for ppi_pair_id in ppi_pair_list: # Iterates through the PPI pair identifiers.
    ...
    # Computes various features and labels related to the surface data from the PLY files.
    for pid in pids:
    ...
    # If applicable (multiple proteins involved and application is masif_ppi_search), computes shape complementarity and saves the results.
    ...
    # Save data only if everything went well: Saves the computed features and labels as NumPy arrays in the precomputation directory.
    for pid in pids: 
