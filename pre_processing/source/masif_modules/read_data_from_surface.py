...
from source.geometry.compute_polar_coordinates import compute_polar_coordinates
...
def read_data_from_surface(ply_fn, params):
    ...
    """
    # Loading the mesh from the PLY file using PyMesh.
    mesh = pymesh.load_mesh(ply_fn)

    # Normals: Retrieves the vertex normals from the mesh.
    ...
    # Compute the angular and radial coordinates. for each vertex 
    ...
    k1 = H + np.sqrt(elem)
    k2 = H - np.sqrt(elem)
    # Compute the shape index using mean and gaussian curvature components
    si = (k1+k2)/(k1-k2)
    ...
    # Iface labels (for ground truth only) initialized to zero if missing     
