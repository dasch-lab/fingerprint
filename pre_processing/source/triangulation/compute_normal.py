...
###
from source.default_config.global_vars import epsilon as eps
...
def compute_normal(vertex, face):
    ...
    # unit normals to the faces --> Compute unit normals to the faces using the cross product. Normalize these normals and ensure the denominators are not too small to avoid division by zero.
    normalf = crossp(
    ...
    # unit normal to the vertex: Compute the unit normals at each vertex by summing the normals of adjacent faces.
    normal = np.zeros((3, nvert))
    ...
    # enforce that the normal are outward otherwise flip the normals
