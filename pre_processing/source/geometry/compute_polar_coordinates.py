...
def compute_polar_coordinates(mesh, do_fast=True, radius=12, max_vertices=200):
    ...
    # Converts the computed distances to a sparse matrix
    d2 = {}
    ...
    # Compute the faces per vertex: dictionary to store the faces associated with each vertex
    idx = {}
    ...
    # Calls the Multi-Dimensional Scaling (MDS) algorithm to compute angular coordinates (theta) for all vertices.
    if do_fast:
    ...
    # Assemble output: Loops through vertices to assemble output arrays based on the computed distances and angular coordinates.
    for i in range(n): 
        dists_i = d2[i]
...
def compute_theta_all(D, vertices, faces, normals, idx, radius):
    ...
    all_theta = []
    for i in range(D.shape[0]): # Loops through each vertex (i) in the mesh.
        if i % 100 == 0:
            print(i)
        # Get the pairs of geodesic distances with a certain radius for each vertex
        neigh = D[i].nonzero()
        ii = np.where(D[i][neigh] < radius)[1]
        neigh_i = neigh[1][ii]
        ...
        # Plane_i: the 2D plane for all neighbors of i (Applies MDS to reduce the dimensionality of the pairwise distances to a 2D plane)
